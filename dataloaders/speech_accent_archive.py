...
class CSVDataset(Dataset):
    ...
    CSV format: filename, country, split
    Audio path: root_dir/recordings/filename.mp3
    """

    def __init__(self,
                 root_dir: str,
                 csv_filename: str = 'speakers_filtered.csv',
        ...
        # First pass: collect all accents to build mapping
        all_accents = set()
        all_rows = []
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if 'filename' not in row or 'country' not in row or 'split' not in row:
                    continue
                
                all_accents.add(row['country'])
                all_rows.append(row)
        ...
        for row in all_rows:
            ...
            # Map 'valid' or 'validation' to 'val' if needed, or just match exactly
            # Assuming CSV has standard split names. If not, we might need normalization.
            # The user said "split用于划分数据", let's assume it contains 'train', 'val', 'test' or similar.
            ...
            if row_split == target_split:
                ...
                audio_path = os.path.join(root_dir, 'recordings', filename)
                self.samples.append({
                    'path': audio_path,
                    'accent': row['country'],
                    'accent_index': self.accent_to_index[row['country']],
                    # 'speaker': row.get('speaker', '') # CSV doesn't seem to have speaker column based on prompt, but maybe filename implies it?
                })
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        try:
            ...
            # Usually better to raise error so we know data is missing
...
def collate_fn(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    ...
    if num_accents is None:
        raise ValueError("Batch items missing 'num_accents'. Ensure CSVDataset returns accent metadata.")
...
def get_dataloader(root_dir: str,
    ...
    ds = CSVDataset(root_dir, augment=augment, normalize=False, **dataset_kwargs)
    if shuffle is None:
